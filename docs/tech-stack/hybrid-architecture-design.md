...
export class UserRegistrationUseCase {
  ...
  async execute(dto: UserRegistrationDto): Promise<RegistrationResult> {
    ...
    if (!dto.roleIds || dto.roleIds.length === 0) {
      const assignRoleCommand = new AssignDefaultRoleCommand(
    ...
    // 4. 发送欢迎邮件
    const sendWelcomeCommand = new SendWelcomeEmailCommand(
      createUserCommand.userId,
    );
...
export class User extends AggregateRoot {
  ...
  update(updateData: Partial<UpdateUserDto>): void {
    ...
    // 发布用户更新事件
    this.apply(new UserUpdatedEvent(this.id, updateData));
  }
...
export class Organization extends AggregateRoot {
  ...
  static create(name: string, code: string, tenantId: string): Organization {
  ...
  update(updateData: Partial<UpdateOrganizationDto>): void {
    ...
    // 发布组织更新事件
    this.apply(new OrganizationUpdatedEvent(this.id, updateData));
  }
...
export class UserDeletedHandler implements IEventHandler<UserDeletedEvent> {
  ...
  async handle(event: UserDeletedEvent): Promise<void> {
    ...
    // 2. 发送删除通知
    await this.notificationService.sendAccountDeletionNotification(
      event.userId,
    );
  ...
  implements IEventHandler<OrganizationCreatedEvent>
{
  ...
  async handle(event: OrganizationCreatedEvent): Promise<void> {
    ...
    // 3. 记录审计日志
    await this.auditService.logOrganizationCreation(
      event.organizationId,
      event.name,
    );
  }
  ...
  implements IEventHandler<OrganizationUpdatedEvent>
{
  ...
  async handle(event: OrganizationUpdatedEvent): Promise<void> {
    ...
    // 3. 记录审计日志
    await this.auditService.logOrganizationUpdate(
      event.organizationId,
      event.updateData,
    );
  }
...
_这是第三部分，包含领域层实现和事件处理。后续部分将包含数据设计、性能优化、部署配置等内容。_
