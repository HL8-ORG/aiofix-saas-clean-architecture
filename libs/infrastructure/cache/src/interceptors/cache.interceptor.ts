...
export class CacheInterceptor implements NestInterceptor {
  ...
  async intercept(
    ...
    const cacheEvict = this.reflector.get<string | string[] | Function>(
      CACHE_EVICT_METADATA,
      handler,
    );
   ...
   * @returns 缓存键
   */
  private generateCacheKey(cacheKey: string | Function, args: any[]): CacheKey {
    const keyString =
  ...
  private async handleCacheEviction(
    next: CallHandler,
    args: any[],
    cacheEvict?: string | string[] | Function,
