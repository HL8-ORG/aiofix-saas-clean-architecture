...
import {
  PinoLoggerService,
  LogContext,
} from '../../../infrastructure/logging/src';
...
export class MemoryCacheService implements ICacheService, OnModuleDestroy {
  ...
  constructor(
    ...
    private readonly userConfig: MemoryCacheConfig,
    ...
    // 设置默认配置
    this.config = {
      defaultTtl: userConfig.defaultTtl ?? 300000, // 5分钟
      maxSize: userConfig.maxSize ?? 1000,
      defaultStrategy: userConfig.defaultStrategy ?? CacheStrategy.LRU,
      cleanupInterval: userConfig.cleanupInterval ?? 60000, // 1分钟
      enableCompression: userConfig.enableCompression ?? false,
      enableEncryption: userConfig.enableEncryption ?? false,
    };
    ...
    this.logger.info(
      `MemoryCacheService initialized with config: ${JSON.stringify(this.config)}`,
      LogContext.CACHE,
    );
  }
  ...
  async get<T = any>(key: CacheKey): Promise<T | null> {
    try {
      ...
      this.logger.error(
        `Error getting cache value: ${error.message}`,
        LogContext.CACHE,
        undefined,
        error,
      );
  ...
  async set<T = any>(
    ...
    try {
      ...
      this.logger.error(
        `Error setting cache value: ${error.message}`,
        LogContext.CACHE,
        undefined,
        error,
      );
  ...
  async delete(key: CacheKey): Promise<boolean> {
    try {
      ...
      this.logger.error(
        `Error deleting cache value: ${error.message}`,
        LogContext.CACHE,
        undefined,
        error,
      );
  ...
  async exists(key: CacheKey): Promise<boolean> {
    try {
      ...
      this.logger.error(
        `Error checking cache key existence: ${error.message}`,
        LogContext.CACHE,
        undefined,
        error,
      );
