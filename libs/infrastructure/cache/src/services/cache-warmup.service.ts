...
export class CacheWarmupService implements OnModuleInit {
  ...
  async warmup(itemIds?: string[]): Promise<WarmupResult[]> {
    ...
    try {
      ...
      this.logger.info(
        `Warmup completed in ${totalTime}ms. Success: ${results.filter(r => r.success).length}/${results.length}`,
        LogContext.CACHE,
      );
      ...
      this.logger.error('Warmup failed', LogContext.CACHE, undefined, error);
      return [];
    } finally {
