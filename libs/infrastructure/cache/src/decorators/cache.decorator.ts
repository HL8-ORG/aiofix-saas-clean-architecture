...
export const Cacheable = (
  ...
  return (target: any, propertyKey: string, descriptor: PropertyDescriptor) => {
    ...
    // 应用选项装饰器（如果提供）
    if (options) {
      CacheOptions(options)(target, propertyKey, descriptor);
    }
...
export const CacheEvictable = (
  ...
  return (target: any, propertyKey: string, descriptor: PropertyDescriptor) => {
    // 应用键失效装饰器（如果提供）
    if (keys) {
      CacheEvict(keys)(target, propertyKey, descriptor);
    }

    // 应用全部失效装饰器（如果提供命名空间）
    if (namespace) {
      CacheEvictAll(namespace)(target, propertyKey, descriptor);
    }
 ...
 * @returns {Function} 装饰器函数
 */
export const CacheKeyFromArgs = (argNames: string[], prefix: string = '') => {
  return CacheKey((args: any[]) => {
 ...
 * @returns {Function} 装饰器函数
 */
export const CacheKeyFromMethod = (prefix: string = '') => {
  return CacheKey((args: any[]) => {
    const argsHash =
      args.length > 0
...
export const CacheSync = (
  ...
  return (target: any, propertyKey: string, descriptor: PropertyDescriptor) => {
    ...
    descriptor.value = function (...args: any[]) {
      ...
      const cacheService = this.cacheService || this.cacheManager;
