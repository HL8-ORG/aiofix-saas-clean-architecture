 ...
 * @description
 * 获取数据库日志配置选项，根据环境变量配置日志级别和输出方式。
 *
 * 主要原理与机制如下：
 * 1. 根据DB_LOGGING环境变量决定是否启用日志
 * 2. 启用时配置error、warn、query日志级别
 * 3. 使用advanced-console作为日志器
 * 4. 支持开发环境的详细日志输出
 *
 * 功能与业务规则：
 * 1. 日志配置生成
 ...
 * 4. 环境适配
 *
 * @returns {object} 返回日志配置对象
 */
function getLoggingOptions() {
  return {
    logging:
      process.env.DB_LOGGING === 'true' ? ['error', 'warn', 'query'] : false,
