...
export class PinoLoggingMiddleware implements NestMiddleware {
  ...
  use(req: FastifyRequest, res: FastifyReply, next: () => void): void {
    ...
    res.send = function (payload?: any): FastifyReply {
      ...
      // 异步记录日志，不阻塞响应
      setImmediate(() => {
        self.logResponse(responseLogData);
      });
  ...
  private extractTenantId(req: FastifyRequest): string | undefined {
    ...
    // 从查询参数中提取
    const queryTenantId =
      (req.query as any)?.tenantId || (req.query as any)?.tenant_id;
    if (queryTenantId) return String(queryTenantId);

    // 从请求体中提取
    const bodyTenantId =
      (req.body as any)?.tenantId || (req.body as any)?.tenant_id;
  ...
  private extractUserId(req: FastifyRequest): string | undefined {
    ...
    // 从查询参数中提取
    const queryUserId =
      (req.query as any)?.userId || (req.query as any)?.user_id;
    if (queryUserId) return String(queryUserId);

    // 从请求体中提取
    const bodyUserId = (req.body as any)?.userId || (req.body as any)?.user_id;
  ...
  private logResponse(data: ResponseLogData): void {
    ...
    const logMethod = this.logger[
      logLevel as keyof PinoLoggerService
    ] as Function;
