/**
 * @file tenant-created.event.ts
 * @description 租户创建事件
 *
 * 该文件定义了租户创建事件，当租户被创建时触发。
 * 该事件包含租户的基本信息和创建上下文。
 */
...
export class TenantCreatedEvent extends BaseEvent {
   ...
   * @description 反序列化事件
   * @param data 事件数据
   * @returns {TenantCreatedEvent} 租户创建事件实例
   */
  static deserialize(data: SerializedEventData): TenantCreatedEvent {
    const eventData = data.metadata.data as TenantCreatedEventData;
